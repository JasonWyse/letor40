...
def validation():
	...
	for fold_number in [1,2,3,4,5]:
		...
		tc=255
		round=500		
		for r in range(1,round+1,20):
			#for rate in [0.00001,0.0001,0.001,0.01]:
			print os.popen(command_train%(train,2,model,r,tc))
		...
		F_pars=open(os.path.join(temp_pth_f,'bestPars'),'a')
		F_pars.write('Fold%d\n%s\n'%(fold_number,BestRound))
		
		#save the best performance 
		ipos1='Fold%d'%fold_number		
		#ipos=0
		Dict={'test':test,'vali':vali,'train':train}
		#within the same fold, train.txt vali.txt test.txt share the same best model
		best_model=final_best_model
		for subset_name in ['test','vali','train']:
			best_score=os.path.join(temp_pth_f,subset_name+'_score')			
			#print os.popen(command_train%(train,2,best_model,BestRound,tc))
...
command_train='java -jar '+tls+' -train %s -ranker %d -save %s -round %d -tc %s'
