...
def EvalQuery(pHash):
	...
	for qid in qids:
		...
		docids =sorted(tmpDid,key=lambda d:float(d[1]['pred']),reverse=True)#reverse=True ,means descending order
		"""if qid[0]=='16625':
			FOUT.write(qid[0])
...
def ReadInputFiles(fnFeature, fnPred,dataset):
	...
	while lnFea!='':		
		...
		#the next line read MQ2008 or Mq2007 dataset
		matchObj  = re.match(r'^([Mm][Qq]\d+)',dataset)
		if matchObj:#data in the form of MQ2007 dataset format
			m = re.match(r'^(\d+) qid\:([^\s]+).*?\#docid = ([^\s]+) inc = ([^\s]+) prob = ([^\s]+)$',lnFea)
		else:# data in the form of OHSUMED dataset format
			m = re.match(r'^(\d+) qid\:([^\s]+).*?\#docid = ([^\s]+)$',lnFea)
		...
		if m:
			...
			temp_did={'label':label,'pred':predScore,'lineNum':lineNum}
			#print temp_did
